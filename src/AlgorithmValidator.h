...
public:
    ...
    bool validateShipDims(unsigned maximalHeight, unsigned x, unsigned y, unsigned numOfFloors);
    bool validateShipXYCords(unsigned width, unsigned length, unsigned x, unsigned y);
    bool validateShipPlanFloorsFormat(const std::vector<std::vector<std::string>> &shipFloors);
    ...
    bool validateSamePortInstancesConsecutively(const std::vector<SeaPortCode> &routeVec);
    bool validatePortFormat(const SeaPortCode &port);
    bool validateOpenReadShipRouteFileAltogether(const std::string &shipRouteFilePath);
    bool validateAmountOfValidPorts(const std::vector<SeaPortCode> &routeVec);
